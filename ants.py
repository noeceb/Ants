...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if current_place.bees and throw >= self.min_range and throw <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        bees_in = list(self.place.bees)
        armor_left = self.armor
        armor_left -= amount
        if armor_left <= 0:
            [counter.reduce_armor(self.damage) for counter in bees_in]
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        def motivate(ant):
            inspired_before = False
            for bug in self.inspired:
                if ant is bug:
                    inspired_before = True
            if not inspired_before:
                self.inspired.append(ant)
                ant.damage *= 2


        if self.true_queen:
            ScubaThrower.action(self, colony)
            loc_before_queen = self.place.exit
            while loc_before_queen != None:
                ant = loc_before_queen.ant
                if ant != None:
                    if ant.is_container:
                        motivate(ant)
                        if ant.contained_ant != None:
                            motivate(ant.contained_ant)
                    else:
                        motivate(ant)
                loc_before_queen = loc_before_queen.exit
        else:
